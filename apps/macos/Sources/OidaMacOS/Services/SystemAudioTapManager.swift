...
enum SystemAudioTapCaptureError: LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case .dataDirectoryUnavailable:
            return "Could not prepare the oida data directory for temporary audio storage."
        case .unsupportedFormat:
...
final class SystemAudioTapManager: NSObject, @unchecked Sendable {
    ...
    // report it by its real bundle id rather than the previous hardcoded "oida" guess.
    private static let excludedProcessIdentifiers = [Bundle.main.bundleIdentifier ?? "org.sonicfield.oida"]
    ...
    private func startScreenCaptureKit() async throws {
        ...
        guard let display = content.displays.first else {
            throw NSError(domain: "oida.system-audio-tap", code: 1, userInfo: [NSLocalizedDescriptionKey: "No capturable display is available."])
        }

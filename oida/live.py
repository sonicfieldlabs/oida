...
from oida.config import data_dir, uploads_dir
from oida.contracts import SourceType, audio_segment_from_path, source_for_path, to_dict
from oida.dsp import AudioData, inspect_path, load_audio
from oida.storage import write_json_atomic

CAPTURE_LAST_PATTERN = "*-oida-capture-last-*s.wav"
...
class LiveManager:
    ...
    def capture_last(self, session_id: str, seconds: float = 10.0) -> dict[str, Any]:
        ...
        output_path = output_dir / f"{stamp}-oida-capture-last-{math.ceil(capture_seconds)}s.wav"
